...
def classify_failure(evidence: FailureEvidence) -> FailureClassification:
    ...
    if evidence.retry_passed:
        return FailureClassification(
            ...
            confidence=0.82,
            reason=(
                "The same revision and environment passed on retry; "
                "isolate and investigate timing or shared state."
            ),
    ...
    if any(hint in message for hint in _ENVIRONMENT_HINTS) or any(
        ...
        return FailureClassification(
            ...
            confidence=0.86,
            reason=(
                "Connectivity, browser lifecycle, or upstream service evidence "
                "indicates an environment failure."
            ),
            test_code_change_allowed=False,
        )
    ...
    if any(hint in message for hint in _TEST_HINTS):
        return FailureClassification(
            ...
            confidence=0.84,
            reason=(
                "The evidence points to locator, fixture, collection, "
                "or test implementation behavior."
            ),
            test_code_change_allowed=True,
        )
