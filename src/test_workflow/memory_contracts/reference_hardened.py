...
from .reference import _synchronized


class DeterministicMemoryReference(_ReferenceBase):
    """Security-hardened M1A reference adapter.

    The approved idempotency contract returns the original result for the same
    authenticated CAS request. Current permission is re-evaluated before replay,
    a new immutable revision starts at CANDIDATE, and forgotten logical IDs
    cannot accept new content.
    """
