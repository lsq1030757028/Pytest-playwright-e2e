...
| Immutable Revision | 原 Revision 被原地修改 | Model validation + CAS integration |
| Session boundary | 原始会话自动成为 Memory | Required formation/provenance validation |
| Namespace isolation | 跨项目或跨 Agent 泄漏 | Negative contract + proof |
...
| Promotion | Memory 变成受保护权威 | Promotion contract + scoped query integration |
...
| Compatibility | 不兼容 Procedure/Skill 被执行 | Unit + effective-read filter |
...
- **Replay/Adversarial**：十项 Proof、Manifest、独立重放和篡改拒绝；
