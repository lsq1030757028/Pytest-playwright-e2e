...
> Addendum ID: `ADDENDUM-HOURLY-GITHUB-RELAY-TERMINATION@0.1.1`  
...
1. **Pilot limit** — bounds the control-plane experiment that proves scheduling, context recovery, Chat reporting, GitHub audit and concurrency control.
2. **Program completion limit** — bounds any later production Relay to the currently authorized M1–M3 delivery program.

An hourly invocation is one execution window, not a promise that one roadmap step or Work Item will finish within one hour. Complex Work Items may span multiple Runs and must be resumed from their durable checkpoint.

## 2. Current Pilot hard limit

The current Pilot scheduled task has a maximum of **8 hourly invocations**.

For the current activation window, the intended Beijing-time occurrences are:
...
The 8-Run cap validates the Relay control plane only. It is not the M1–M3 product-development budget, is not tied to roadmap step count, and does not require a Campaign or Work Item to finish within the Pilot.

The Pilot may be renewed or promoted only through a new Owner instruction after reviewing the accumulated Run evidence.
...
1. finish the current safe checkpoint and release the Lease;
...
Pilot completion does not mean the active product Campaign, Work Item or M1–M3 Program is complete.
...
- a Work Item that legitimately requires more than one Run.
